...
const Blog = () => {
  ...
  const handleRating = (postId, value) => {
    setPosts(posts.map(post => post._id === postId ? { ...post, rating: value } : post));
  };

  if (loading) return <p className="text-center mt-10 text-lg">Cargando posts...</p>;

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-4xl font-extrabold mb-8 text-green-700 text-center">Blog</h1>
      ...
      >
        <h2 className="text-2xl font-semibold text-gray-700 mb-4">Crear nuevo post</h2>
        <input
          type="text"
          placeholder="Título"
          value={title}
      ...
      <div className="space-y-6">
        {posts.map((post) => (
          ...
          >
            ...
            <p className="text-gray-500 text-sm mb-2">{new Date(post.fecha).toLocaleDateString()}</p>
            ...
            <div className="flex items-center">
              ...
              <span className="ml-2 text-gray-600 font-medium">{post.rating} / 5</span>
            </div>
          </div>
        ))}
      </div>
